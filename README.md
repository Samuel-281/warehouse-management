...
这是一个面向两级仓库业务的货物管理软件项目。项目当前阶段已经完成需求梳理、使用说明书、PDF 文稿和第一版可交互网页原型。
...
1. 确认正式技术架构：数据库、后端框架、部署方式和权限模型。
2. 设计真实数据模型，以单件条码库存记录和库存流水为核心。
3. 把基础资料从只读演示改为可维护功能。
4. 将入库、出库、销售退回、库存查询接入后端接口。
5. 完成 PDA 端核心扫码闭环。
6. 补充业务测试、权限测试和部署文档。
