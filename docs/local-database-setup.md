...
超级管理员登录后可进入“系统维护”，查看最近操作日志，并执行业务数据清空。网页端清空需要输入 `确定重置`，且只允许超级管理员执行。当前项目要求该入口在正式运行模式中常设开放；执行后不会写入测试数据，使用前应确认目标数据库。
...
正式版网页端保留“清空业务数据”的管理能力，但不再重新写入演示/测试数据，且必须满足以下约束：
